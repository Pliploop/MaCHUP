...
class CustomAudioDataset(Dataset):
    def __init__(self, data_dir, file_list = None, augmentations=None, transform = True, target_sample_rate = 24000, target_length = 20, train = True, n_augmentations = 1, extension = "wav"):
        self.data_dir = data_dir
        self.extension = extension
        
        self.file_list = self.get_file_list(data_dir=data_dir, extenstion=extension)
    ...
    def __getitem__(self, idx):
        ...
        # Do the split here
        waveform = torch.cat(torch.split(empty, self.target_n_samples_one, dim = 1)).unsqueeze(1)
...
class CustomAudioDataModule(pl.LightningDataModule):
    def __init__(self, train_data_dir = None, val_data_dir = None, batch_size=64, num_workers = 0, target_sample_rate=24000, target_length = 20, validation_split = None, n_augmentations= 1):
        super().__init__()
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            self.train_dataset = CustomAudioDataset(data_dir = self.train_data_dir, augmentations=self.train_transforms, target_sample_rate = self.target_sample_rate, target_length = self.target_length, n_augmentations = self.n_augmentations )
            ...
            if self.validation_split is not None:
                self.train_dataset, self.val_dataset = torch.utils.data.random_split(self.train_dataset,[1-self.validation_split,self.validation_split])
