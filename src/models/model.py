...
class MaCHUP(LightningModule):
    ...
    def forward(self, x):
        ...
        if self.first_run or codes.shape[-1]*B*N != self.extended_batch_size:
            self.contrastive_matrix = self.get_contrastive_matrix(T = codes.shape[-1], B = B, N = N, W = self.window_size, device = codes.device)
    ...
    def pad_codes_to_length(self, codes, padding_mask):
        ...
        target_padding_mask_shape = (padding_mask.shape[0], self.sequence_len+1)
        target_codes_shapes = (codes.shape[0], codes.shape[1], self.sequence_len+1)
        if codes.shape[-1] > self.sequence_len:
            padding_mask = padding_mask[:, : self.sequence_len]
            codes = codes[:, :, : self.sequence_len]
        else:
            new_padding_mask = torch.ones(target_padding_mask_shape, device = codes.device).bool()
            new_codes = torch.full(target_codes_shapes, self.pad_special_token, device = codes.device)
    ...
    def get_contrastive_matrix(self,B, T, N, W, device = None):
        ...
        for k in range(1, window_width):
            window_mask += torch.diag(torch.ones((total_samples - k),device = device), k)
            window_mask += torch.diag(torch.ones((total_samples - k), device = device), -k)
    ...
    def get_contrastive_loss(self,encoded,contrastive_matrix_removed):
        ...
        loss = self.supconloss(encoded,positive_mask = positive_mask, negative_mask = negative_mask, neutral_mask = neutral_mask)
        class_loss = self.supconloss(only_class_tokens,positive_mask = positive_class, negative_mask = negative_class)
    ...
    def training_step(self, batch, batch_idx):
        ...
        contrastive_loss, contrastive_class_loss = self.get_contrastive_loss(encoded,contrastive_matrix_dict['contrastive_matrix_masked_removed'])
        ...
        self.log("contrastive loss", contrastive_loss, prog_bar=True, sync_dist=True)
        self.log("contrastive global loss", contrastive_class_loss, sync_dist = True)
        ...
        if self.logger is not None and self.global_step %1000 == 0:
            ...
            ax[0].imshow(contrastive_matrix_dict['original_contrastive_matrix'].cpu(), cmap="plasma")
            ax[1].imshow(contrastive_matrix_dict['contrastive_matrix_masked_blackout'].cpu(),cmap="plasma")
            ax[2].imshow(contrastive_matrix_dict['contrastive_matrix_masked_removed'].cpu(),cmap="plasma")
            
            self.logger.log_image("contrastive matrix, masked and unmasked", [wandb.Image(fig)])
            plt.close(fig)
    ...
    def validation_step(self, batch) -> STEP_OUTPUT:
        ...
        self.log("val_crossentropy_simple", masked_loss, prog_bar=True, sync_dist=True)
        self.log("val_contrastive loss", contrastive_loss, prog_bar=True, sync_dist=True)
        self.log("val_contrastive global loss", contrastive_class_loss, sync_dist = True)
