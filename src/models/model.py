...
from src.models.utils import NormalizedMultiTaskSplitter
...
class MaCHUP(LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.window_size = window_size * self.encodec.fps
        ...
        self.loss_importance_dict = {
            "contrastive_loss": self.contrastive_to_masked_ratio, "masked_loss": 1 - self.contrastive_to_masked_ratio
        }
        
        self.splitter = NormalizedMultiTaskSplitter(self.loss_importance_dict)
    ...
    def create_padding_mask(self, codes, original_lens):
        ...
        if original_lens is not None:
            ...
            encoded_lens = original_lens // self.encodec.hop_length
    ...
    def training_step(self, batch, batch_idx):
        ...
        if masked_unmasked["masked_loss"].isnan().any():
            masked_loss = masked_unmasked['unmasked_loss']
        else:
...
class MultiEncoderMaCHUP(LightningModule):
    def __init__(
        self,
        encoder: nn.Module,
        feature_decoder: nn.Module,
        ...
        **kwargs,
        ...
        self.supconloss = MySupConLoss(temperature=contrastive_temperature)
        self.feature_decoder_proj_head = nn.Sequential(
            nn.Linear(self.d_model, self.d_model),
            nn.ReLU(),
            nn.Linear(self.d_model, 128)
        )
        
        self.target_encoder_proj_head = nn.Sequential(
            nn.Linear(self.d_model, self.d_model),
            nn.ReLU(),
            nn.Linear(self.d_model, 128)
        )
    ...
    def forward(self, x):
        ...
        target_encoded, _, _, _, _ = self.transformer_encoder.finetune_forward(
            codes=codes,
            padding_mask=padding_mask,
            mask_before=self.mask_before,
            contrastive_matrix=self.contrastive_matrix,
        ...
        reconstruction_decoded_logits = self.reconstruction_decoder(
            unmasked_encoded, padding_mask=encoded_padding_mask
        )  # SHAPE : [B,n_q,T,card]
        
        feature_decoded_logits = self.feature_decoder(
            unmasked_encoded, padding_mask=encoded_padding_mask
        )  # SHAPE : [B,n_q,T,card]

        decoded_projected = self.feature_decoder_proj_head(feature_decoded_logits)
        encoded_projected = self.target_encoder_proj_head(target_encoded)
        ...
        return {
            ...
            "padding_mask": padding_mask,
            "encoded": encoded,
            "decoded_projected": decoded_projected,
            "encoded_projected": encoded_projected,
            "logits": reconstruction_decoded_logits,
            "contrastive_matrix_dict": contrastive_matrix_dict,
        }
    ...
    def training_step(self, batch, batch_idx):
        ...
        print(masked_unmasked["masked_loss"])
        print(masked_unmasked["unmasked_loss"])
        if masked_unmasked["masked_loss"].isnan().any(): ## deals with the case where p = 0 leading to no masked tokens and nan loss
            print("masked loss is nan, setting it to unmasked loss")
